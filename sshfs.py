...
- hosts : web
  ...
  - service:
	...
	# set up client
	commands.getstatusoutput("sudo sshpass -p {0} ssh -o stricthostkeychecking=no -l root  {1}  mkdir /media/{2}/".format(password,clientIP,userName))
	...
	mountF = commands.getstatusoutput("sudo sshpass -p {0} ssh -o stricthostkeychecking=no -l root {1}  sudo sshfs {2}@192.168.43.171:/sshfs/{2}-lv1 /media/{2}/ -o IdentityFile=/root/.ssh/id_rsa".format(password,clientIP,userName))
	if(mountF[0] == 0):
