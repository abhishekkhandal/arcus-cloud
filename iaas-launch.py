...
if osDistro == "1":
	...
	distro = "Null"
	osVariant = ""
ossetup="sudo sshpass -p toor ssh -o stricthostkeychecking=no -l root 192.168.43.204 virt-install --name  {0} --location  /iaas-images/{5}  --os-type linux  --os-variant {6} --memory  {4} --vcpus  {2} --disk  /var/lib/libvirt/images/{0}.qcow2,size={3} --graphics  vnc,listen=0.0.0.0,port=5901  --noautoconsole".format(osVariant,ostype,cpunumber,storagesize,ramsize,distro,osVariant)
